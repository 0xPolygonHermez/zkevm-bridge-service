...
// 		amount := new(big.Int).SetUint64(10000000000000000000)
// 		tokenAddr, token, err := opsman.DeployERC20(ctx, "A COIN", "ACO", "l2")
// 		require.NoError(t, err)
// 		//Mint tokens
...
func TestE2E(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run("ERC20 Test id "+strconv.FormatUint(uint64(testCase.ID), 10), func(t *testing.T) {
			...
			// Send L2 Deposit to withdraw the some funds
			destNetwork = 0
			amount = new(big.Int).SetUint64(6000000000000000000)
			//////////////////////////////////
			err = opsman.ApproveERC20(ctx, tokenWrapped.WrappedTokenAddress, common.HexToAddress("0x9d98deabc42dd696deb9e40b4f1cab7ddbf55988"), amount, "l2")
			require.NoError(t, err)
			////////////////////////////////////
			...
			balance, err = opsman.CheckAccountTokenBalance(ctx, "l1", tokenAddr, &destAddr)
			require.NoError(t, err)
			assert.NotEqual(t, big.NewInt(100000000000000000), balance)
			// Check L2 funds to see that the amount has been reduced
