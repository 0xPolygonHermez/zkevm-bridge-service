...
// AddDeposit provides a mock function with given fields: deposit, dbTx
func (_m *bridgectrlMock) AddDeposit(deposit *etherman.Deposit, dbTx pgx.Tx) error {
	ret := _m.Called(deposit, dbTx)

	var r0 error
	if rf, ok := ret.Get(0).(func(*etherman.Deposit, pgx.Tx) error); ok {
		r0 = rf(deposit, dbTx)
	} else {
		r0 = ret.Error(0)
	}
